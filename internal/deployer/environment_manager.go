...
type Environment struct {
	...
	Secrets     map[string]string      `json:"secrets"` // Encrypted
	Domains     []string               `json:"domains"`
	Resources   ResourceAllocation     `json:"resources"`
	Locked      bool                   `json:"locked"`
	LockedBy    string                 `json:"locked_by,omitempty"`
	LockedAt    *time.Time             `json:"locked_at,omitempty"`
	AccessRoles map[string][]string    `json:"access_roles"` // role -> users
	CreatedAt   time.Time              `json:"created_at"`
	UpdatedAt   time.Time              `json:"updated_at"`
	Metadata    map[string]interface{} `json:"metadata"`
}

// ResourceAllocation defines resource limits for an environment
...
// EnvironmentManager manages deployment environments
...
func (em *EnvironmentManager) CreateEnvironment(ctx context.Context, env *Environment) error {
	...
	// Set default resource allocation based on environment type
...
// CloneEnvironment creates a copy of an environment
func (em *EnvironmentManager) CloneEnvironment(ctx context.Context, sourceID, targetName string, targetType EnvironmentType) (*Environment, error) {
	...
	// Copy secrets (but not production secrets to non-production environments)
	...
	// Adjust resources for non-production environments
...
func (em *EnvironmentManager) PromoteEnvironment(ctx context.Context, sourceID, targetID string) error {
	...
	// Copy variables (excluding environment-specific ones)
	...
	// Don't copy secrets automatically for security
	// Secrets should be managed separately

	return em.UpdateEnvironment(ctx, target)
}

// LockEnvironment locks an environment to prevent changes
...
// SetSecret sets an encrypted secret in an environment
...
// GetSecret retrieves a decrypted secret from an environment
